 ...
 *    Assumes two processors and two fans.
 *    Requires applesmc kernel module to be loaded.
 *
 *  Tested models:
 *    MacBook Pro 6,2 15" 
 */
...
unsigned short get_temp()
{
  ...
  file=fopen("/sys/devices/platform/applesmc.768/temp5_input", "r");
  
  if (!file)
    {
      printf ("Could not recover temperature\n");
      return 80;
    }
  ...
  file=fopen("/sys/devices/platform/applesmc.768/temp7_input", "r");
